...
class SharedDocument(HookProvider):
    ...
    def save_shared_document(self, event: AfterInvocationEvent):
        ...
        shared_document["trader_investment_plan"] = report
...
Investment Plan: {shared_document.get("trader_investment_plan")}
        ...
        return "No relevant past trading decisions memories found."
...
class StoreMemoryHook(HookProvider):
    ...
    def save_memory(self, event: AfterInvocationEvent):
        ...
        self.memory.add_memory(
            memory=f"Trader Decision for {event.agent.state.get('ticker')} on {event.agent.state.get('current_date')}: {report}",
            ticker=event.agent.state.get("ticker"),
        )
